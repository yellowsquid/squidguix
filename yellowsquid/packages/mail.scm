    ...
    (package
      ...
      (source
       (origin
         ...
         (patches (search-patches "libspf2-undefined.patch"))
         (sha256
...
viruses/worms.")
      (home-page "https://www.libspf2.net/index.html")
  ...
  (package
    ...
    (source
     (origin
       ...
       (sha256
        (base32 "0xqzgjqw8hv8wkkw5lamzd3dzb6g01y3gazyxjfjgk71y57asxdy"))))
    (native-inputs (list autoconf automake libtool))
    (build-system gnu-build-system)
    (arguments
    ...
    (description
     "A package providing DMARC report generation and policy
                ...
                (let ((out (assoc-ref outputs "out")))
                  (substitute* "Local/Makefile"
                    ...
                    (("(CONFIGURE_FILE=).*" all var)
                     (string-append var "/etc/exim.conf:" out
                                    "/etc/exim.conf\n"))
                    ...
                    (("# (USE_OPENSSL(|_PC)=.*)" all line)
                     (string-append line "\n"))
                    ;; Enable LMTP
                    (("# (TRANSPORT_LMTP=yes)" all line)
                     line)
                    ;; Enable SPF
                    (("# (SUPPORT_SPF=yes)" all line)
                     line)
                    (("# (LDFLAGS.*spf2)" all line)
                     line)
                    ;; Enable DMARC
                    (("# (SUPPORT_DMARC=yes)" all line)
                     line)
                    (("# (LDFLAGS.*opendmarc)" all line)
                     line)))))
            (add-after 'install 'install-config
              (lambda* (#:key outputs #:allow-other-keys)
                (let ((out (assoc-ref outputs "out")))
                  (mkdir-p (string-append out "/etc"))
                  (copy-file "src/configure.default"
                             (string-append out "/etc/exim.conf"))
